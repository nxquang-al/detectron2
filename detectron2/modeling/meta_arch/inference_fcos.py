...
class ObjectSelector:
    '''
        Select only an object that user has clicked on
    '''
    ...
    def keepOnlySelectedObject(self) -> 'Instances':
        ...
        if self.selected_idx >= self.instances.pred_boxes.__len__() or self.selected_idx <= -self.instances.pred_boxes.__len__():
            return Instances(self.image_shape[:2])
        selected_box = self.instances.pred_boxes[self.selected_idx]
        selected_cls = torch.unsqueeze(self.instances.pred_classes[self.selected_idx], 0)  # 0-d to 1-d tensor
